...
public class LinSolver {
	...
	public static boolean containsNode(ArrayList<Edge> path, int v) {
		for (Edge edge : path) {
			if (edge.getU() == v || edge.getV() == v) return true;
		}
		return false;
	}
	...
	public static int getNbDoubleBounds(int [] hexagonDoubleBounds) {
		...
		for (int i = 0 ; i < hexagonDoubleBounds.length ; i++)
			if (hexagonDoubleBounds[i] == 1) sum ++;
	...
	public static List<ArrayList<Edge>> computeAllPaths(UndirPonderateGraph kekuleStructure, int a, int b) {
		...
		ArrayList<Integer> firstCandidate = new ArrayList<Integer>();
		...
		while (true) {
			...
			if (reachB) {
				List<ArrayList<Edge>> paths = new ArrayList<ArrayList<Edge>>();
	...
	public static List<Edge> computeCircuitCase4(UndirPonderateGraph kekuleStructure, int hexagon) {
		...
		List<Edge> pV1V2 = new ArrayList<Edge>();
		List<Edge> pV3V4 = new ArrayList<Edge>();
		List<Edge> pV5V6 = new ArrayList<Edge>();
		...
		List<Edge> C1 = new ArrayList<Edge>();
		C1.addAll(pV1V2);
		...
		List<Edge> pV2V3 = new ArrayList<Edge>();
		List<Edge> pV4V5 = new ArrayList<Edge>();
		List<Edge> pV6V1 = new ArrayList<Edge>();
		...
		List<Edge> C2 = new ArrayList<Edge>();
		C2.addAll(pV2V3);
	...
	public static void displayUsage() {
		System.err.println("USAGE: ./${EXEC_NAME} ${INSTANCE_NAME}");
	}
